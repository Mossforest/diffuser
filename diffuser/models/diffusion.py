...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def p_sample_loop_atraj(self, shape, cond, verbose=True, return_chain=False, sample_fn=default_sample_fn, **sample_kwargs):
        device = self.betas.device

        batch_size = shape[0]
        x = torch.randn(shape, device=device)
        x = apply_conditioning_atraj(x, cond, self.action_dim)
    ...
    @torch.no_grad()
    def p_sample_loop_sa_traj(self, shape, cond, verbose=True, return_chain=False, sample_fn=default_sample_fn, **sample_kwargs):
        device = self.betas.device
    ...
    @torch.no_grad()
    def conditional_sample_wo_action(self, cond, n_samples, horizon=None, **sample_kwargs):
        '''
            conditions : [ (time, state), ... ]
        '''
        device = self.betas.device
    ...
    @torch.no_grad()
    def conditional_sample_atraj(self, cond, n_samples, horizon=None, **sample_kwargs):
        '''
            conditions : [ (time, (state, action)), ... ]
        '''
        device = self.betas.device
        ...
        return self.p_sample_loop_atraj(shape, cond, **sample_kwargs)

    @torch.no_grad()
    def conditional_sample_sa_traj(self, cond, n_samples, horizon=None, **sample_kwargs):
        '''
            conditions : [ (time, (state, action)), ... ]
        '''
        device = self.betas.device
        ...
        return self.p_sample_loop_sa_traj(shape, cond, **sample_kwargs)
